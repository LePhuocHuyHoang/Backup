...
//            assignRoleToUser(user.getId(),Role.USER);
    ...
    public ResponseEntity<?> forgotPassword(String username) {
        // CHECK nguoi dung co input
        boolean isUserExists = Optional.ofNullable(userRepository.findByUsername(username)).isPresent();
        if(!isUserExists){
            return new ResponseEntity<>(new ApiResponse(Boolean.FALSE, "Account does not exists"), HttpStatus.BAD_REQUEST);
        }
        ...
        if (userRepresentation != null) {
            ...
            return new ResponseEntity<>(new ApiResponse(Boolean.TRUE, "We have sent you an email to change your password " + username), HttpStatus.OK);
        }
