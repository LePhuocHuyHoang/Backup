...
public class BookServiceImpl implements BookService {
    ...
    public ResponseEntity<?> getBook(Long bookId, String username) {
        ...
        Optional avg = Optional.ofNullable(bookRepository.averageRating(bookId)[0]);
        Optional<Object[]> rating = Optional.ofNullable(userRepository.getUserRating(user.getId(), bookId));
        Optional<Object[]> retal = Optional.ofNullable(bookRepository.checkIsBought(user.getId(), bookId));
        boolean isBought = retal.get().length > 0;

        BookDTO bookDTO = new BookDTO(book, avg.isPresent() ? (double) avg.get() : 0, rating.get().length > 0 ?(double)  rating.get()[0] : 0, isBought);
        return ResponseEntity.ok(bookDTO);
    }

    @Override
    public ResponseEntity<?> getBookGuest(Long bookId) {
        boolean exists = bookRepository.existsById(bookId);
