...
const AddNewAuthor = ({ handleAddAuthorSuccess, handleClose, authorId }) => {
    ...
    useEffect(() => {
        const fetchAuthorData = async (authorId) => {
            try {
                ...
                const response = await axios.get(`http://localhost:8098/admin/author?authorId=${authorId}`, {
                    headers: {
                ...
                setDob(dayjs(authorData.dob).format('YYYY-MM-DD'));
                setBio(authorData.bio);
            } catch (error) {
                console.error('Error fetching author data:', error);
                setError('Không thể tải thông tin tác giả để cập nhật');
            }
        };
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.put(`http://localhost:8098/admin/author?authorId=${authorId}`, inputData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            });
