...
END;
--Goi Proc:
EXEC GetFeaturedBooks 12

--Get Top Rentel-Book (Xếp hạng mua sách giảm dần)
Create PROCEDURE GetTopRentedBooks
@NumberOfBooks INT
AS
BEGIN
SET NOCOUNT ON;
...
SET @SQLQuery = '
SELECT TOP (' + CAST(@NumberOfBooks AS NVARCHAR) + ')
b.book_id,
b.book_name,
b.introduce,
b.IBSN,
...
END;
--Goi ham:
EXEC GetTopRentedBooks 21

--Get all bookmark by user

alter PROCEDURE getBookmark 
    @userId INT,
	@offset INT,
	@fetch INT
AS
...
BEGIN
	...
	insert into author_book values(@bookId, @authorId)
END;
...
BEGIN
	...
	insert into book_type values(@bookId, @typeId)
END;
...
CREATE PROC deleteUserByUserName
